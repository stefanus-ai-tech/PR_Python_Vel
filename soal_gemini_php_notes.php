...
for ($x = 0; $x <= 5; $x++){
    ...
    if($x == 1){
      echo "Number {$x} is {$status}, stopping the program \n";
      break;
    } else {
      echo "Number {$x} is {$status} \n";
    }
...
function calculateRectangleArea($length, $width){
  ...
  echo calculateRectangleArea(10,11) , "\n";
?>
```
...
$fruitlist = array("Apple", "Orange", "Mango", "Banana");
...
function printAllElements($array_list){

  foreach($array_list as $object){
    echo $object, "\n";
...
- **PHP Tips:** Access characters using bracket notation (e.g., `$message[0]`). Use `strlen()`
to find the length for
accessing the last character (`$message[strlen($message) - 1]`). Use `substr()` to extract
...
echo $message[0], "\n";
echo $message[strlen($message)-1], "\n";
echo substr($message, 6,3 ), "\n";
...
- **PHP Tips:** Use `strtoupper()`, `strtolower()`, and `strpos()`.
Note that `strpos()` returns the starting position
(0-indexed) or `false` if the substring is not found.

```php
<?php
$mixed_case = "PhP PrOgRaMmInG";
echo strtoupper($mixed_case), "\n";
echo strtolower($mixed_case), "\n";
echo strpos($mixed_case, "PrOg"), "\n";
?>
```
