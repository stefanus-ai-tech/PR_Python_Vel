...
*   **Tujuan:** Memahami cara menghentikan loop sebelum selesai secara normal menggunakan `break`. Mirip dengan contoh Soal 3 Anda.
*   **Tugas:** Buat kode JavaScript menggunakan `loop` (`for`) untuk angka dari 0 hingga 9. Periksa setiap angka. Jika angka tersebut ganjil, cetak pesan bahwa program berhenti dan hentikan loop. Jika genap, cetak bahwa bilangan itu genap. Output yang diharapkan:
    ```
    Bilangan 0 adalah bilangan genap
    Bilangan 1 adalah bilangan ganjil, program berhenti.
    ```
*   **Tips:** Gunakan `if` untuk memeriksa kondisi ganjil. Jika terpenuhi, cetak pesan dan gunakan `break;` untuk keluar dari loop.

---

**Soal 6: Loop Bersarang (Nested Loop) - Membuat Pola Sederhana**

*   **Tujuan:** Mengerti cara kerja loop di dalam loop untuk membuat struktur yang lebih kompleks seperti pola atau tabel.
*   **Tugas:** Buat kode JavaScript menggunakan *nested loop* (loop di dalam loop) untuk menghasilkan pola bintang segitiga siku-siku seperti berikut:
    ```
    *
    **
    ***
    ...
    *****
    ```
*   **Tips:** Loop luar akan mengontrol jumlah baris. Loop dalam akan mengontrol jumlah bintang yang dicetak per baris. Gunakan `console.log()` untuk pindah baris setelah loop dalam selesai. Anda mungkin perlu membangun string di dalam loop dalam sebelum mencetaknya.
...
*   **Tugas:** Buat sebuah *fungsi* JavaScript bernama `sapaPengguna` yang menerima satu argumen (parameter) berupa `nama` (string). Fungsi ini harus mencetak pesan sapaan ke console.
    ...
    ```
*   **Tips:** Gunakan keyword `function` untuk mendefinisikan fungsi. Gunakan parameter di dalam fungsi. Panggil fungsi dengan memberikan nilai argumen.

---

**Soal 8: Fungsi dengan Nilai Kembali (Return Value)**

*   **Tujuan:** Memahami fungsi yang tidak hanya melakukan aksi, tetapi juga mengembalikan sebuah nilai hasil perhitungan atau pemrosesan.
*   **Tugas:** Buat sebuah fungsi JavaScript bernama `hitungLuasPersegiPanjang` yang menerima dua argumen: `panjang` dan `lebar`. Fungsi ini harus *mengembalikan* (return) hasil perhitungan luasnya.
...
*   **Tugas:**
    ...
    Pisang
    ```
*   **Tips:** Array di JavaScript dibuat dengan kurung siku `[]`. Anda bisa mengakses elemen berdasarkan indeks (mulai dari 0) atau menggunakan loop `for...of` yang lebih modern untuk iterasi elemen secara langsung.

---

**Soal 10: Fungsi yang Memproses Array**

*   **Tujuan:** Menggabungkan fungsi dan array. Membuat fungsi yang menerima array sebagai input dan melakukan sesuatu dengannya.
*   **Tugas:** Buat sebuah fungsi bernama `cetakSemuaElemen` yang menerima satu argumen berupa `array`. Fungsi ini harus mencetak semua elemen dalam array tersebut ke console, sama seperti Soal 9.
    Contoh pemanggilan:
    ```javascript
    let angka = [10, 20, 30];
...
1.  **Nama Variabel dan Fungsi yang Jelas:** Gunakan nama yang deskriptif (misal: `hitungLuas` lebih baik dari `hl`, `namaPengguna` lebih baik dari `n`). Gunakan *camelCase* (kata kedua dan seterusnya diawali huruf kapital: `namaVariabelContoh`).
2.  **Komentar:** Tambahkan komentar (`//` untuk satu baris, `/* */` untuk banyak baris) untuk menjelaskan bagian kode yang mungkin kompleks atau tujuannya tidak langsung jelas.
3.  **Konsistensi Kode:** Jaga gaya penulisan kode Anda konsisten (misal: penempatan kurung kurawal `{ }`, penggunaan spasi).
4.  **Gunakan `const` dan `let`:** Gunakan `const` untuk variabel yang nilainya tidak akan berubah setelah diinisialisasi. Gunakan `let` untuk variabel yang nilainya mungkin perlu diubah. Hindari `var` sebisa mungkin di kode modern.
...
Tujuan latihan ini adalah memperdalam pemahaman tentang konsep JavaScript lanjutan, struktur data, pola desain umum, penanganan operasi asinkron, dan penulisan kode yang bersih, modular, dan efisien.
...
*   **Tugas:** Diberikan sebuah array objek yang merepresentasikan daftar produk:
    ...
    2.  Menggunakan `map` pada hasil filter untuk membuat array baru yang hanya berisi nama dan harga produk elektronik tersebut.
    3.  Menggunakan `reduce` pada array produk elektronik (hasil filter langkah 1) untuk menghitung total nilai inventaris (harga * stok) semua produk elektronik.
...
*   **Contoh Output (berdasarkan data di atas):**
    ...
    ```
*   **Tips:** Gabungkan pemanggilan `filter`, `map`, dan `reduce` secara berantai jika memungkinkan untuk kode yang lebih ringkas.
